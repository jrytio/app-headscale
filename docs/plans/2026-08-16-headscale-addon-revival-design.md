...
- Every longrun runs under a dedicated non-root user via `s6-setuidgid`; `/data` subdirectories are chowned per service. Only s6 init itself is root. Exception: the nginx MASTER process stays root — as non-root it cannot open s6's root-owned stdout pipe (/proc/1/fd/1) for addon-log output; all worker processes (which handle every untrusted request) drop to the nginx user, and the AppArmor profile already provisions exactly this (setuid/setgid + /proc/1/fd/* w). A fully non-root alternative via an s6-log consumer pipeline is noted as future work.
- **AppArmor deviation**: `apparmor.txt` is a single flat profile, not four `cx ->`-transitioned per-service child profiles. A nested-child-profile revision was tried first and passed local (non-enforcing) testing, but under Supervisor's real AppArmor enforcement in CI it left the addon crash-looping (`s6-ipcserver-socketbinder: fatal: unable to create socket: Permission denied`, every `init-*` oneshot blocked forever) even after targeted fixes; the nested profile's own syntax and Supervisor's slug-rename step were independently verified to be correct, but per-service isolation under enforcement couldn't be proven or debugged without a real AppArmor-enforcing kernel (unavailable locally). The flat shape is the one most community add-ons use and is known-compatible with Supervisor's loader; the meaningful restrictions (no raw sockets, no mount/umount, minimal capabilities, no `/sys` or `/proc/sys/kernel` writes) are preserved as a union across all four services rather than isolated per service.
